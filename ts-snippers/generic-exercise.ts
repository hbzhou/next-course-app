...
function mapDict<T, S>(inputDict: Dict<T>, mapFunc: (item: T, name: string) => S): Dict<S> {
  const outputDict: Dict<S> = {};
  Object.keys(inputDict).forEach((key) => {
    const output = mapFunc(inputDict[key], key);
    outputDict[key] = output;
  });
  return outputDict;
}
// Array.prototype.filter, but for Dict
function filterDict<T>(inputDict: Dict<T>, predicate: (item: T) => boolean): Dict<T> {
  const outputDict: Dict<T> = {};
  Object.keys(inputDict).forEach((key) => {
    if (predicate(inputDict[key])) {
...
function reduceDict<T>(inputDict: Dict<T>, reduce: (current: number, item: T) => number, initial: number): number {
  let total: number = initial;
  Object.keys(inputDict).forEach((key) => {
    total = reduce(total, inputDict[key]);
  });
  return total;
}
